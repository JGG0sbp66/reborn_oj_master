    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" :loading="creating" @click="submitProblem">创建</el-button>
        </span>
      </template>
    </el-dialog>
      ...
      <div class="testcase-upload-content">
        <el-form label-position="top">
          <el-form-item label="题目ID">
            <el-input v-model="createdProblemUid" disabled class="problem-id-input"></el-input>
          </el-form-item>
      ...
      if (err.status !== null) {
        ...
        if (status === 400) {
          ...
          errorMessage = err.message
            ? `服务器内部错误 (500): ${err.message}`
            : '服务器内部错误 (500)';
        } else {
          errorMessage = err.message
            ? `服务器错误 (${status}): ${err.message}`
            : `服务器错误 (${status})`;
        }
      } else {
        // 请求已发送但没有收到响应
...
const handleUploadError = (error: any, file: any) => {
  ...
  // errorMessage 未必是 JSON，解析失败时直接展示原文
  let displayMessage = errorMessage;
  try {
    const errorMessageObject = JSON.parse(errorMessage);
    displayMessage = errorMessageObject.message + '，详情：' + errorMessageObject.details;
  } catch {
    // 非 JSON 格式，保持原文
  }
  // 使用自定义提示组件显示错误信息
  props.alertBoxRef?.show(displayMessage, 2);

            ...
            <tr v-for="(rank, index) in raceRank.race_rank" :key="rank.user_id" class="rank-row">
              <td class="column-rank">
                <div class="rank-number-wrapper">
                  <div class="rank-number" :class="getRankClass(Number(index) + 1)">
                    {{ Number(index) + 1 }}
                  </div>
                </div>
              </td>
                ...
                >
                  ...
                  <template v-if="rank.problem_stats[problem.problemId]?.solved">
                    <div class="penalty-time">
                      {{ rank.problem_stats[problem.problemId].penalty_time }}
                    </div>
                    <div class="submit-count">
                      {{ rank.problem_stats[problem.problemId].submit_count }}
                      {{ rank.problem_stats[problem.problemId].submit_count > 1 ? 'trys' : 'try' }}
                    </div>
                  </template>
...
// 获取题目状态的样式类
const getProblemStatusClass = (problem: any) => {
...
const handleAvatarError = (e: Event) => {
  ...
  if (target.parentElement) {
    target.parentElement.innerHTML = '<el-icon><User /></el-icon>';
  }
};

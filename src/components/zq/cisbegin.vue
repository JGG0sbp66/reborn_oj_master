...
const signedUpLocally = ref(false); // 报名成功后的本地即时反馈（不直接改 props）
const isSignedUp = computed(
  () => signedUpLocally.value || props.raceInfo?.value?.race_info?.is_registered === '已报名'
);
const alertBoxRef = ref<InstanceType<typeof AlertBox> | null>(null);
...
const handleSignup = async () => {
  ...
  try {
    loading.value = true;
    const data = await raceApi.registerRace(Number(props.uid));

    if (data.success) {
      // 本地即时反馈，父组件刷新后以其数据为准
      signedUpLocally.value = true;
    ...
    alertBoxRef.value?.show(error.response?.data?.message || '报名失败，请稍后重试', 2);
  } finally {
    loading.value = false;
  }
};

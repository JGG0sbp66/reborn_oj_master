<template>
  ...
  <ProblemCreate ref="problemCreateRef" :alert-box-ref="alertBox" @refresh-data="fetchData" />
  <ProblemEdit ref="problemEditRef" :alert-box-ref="alertBox" @refresh-data="fetchData" />
  <ProblemDetail
    ref="problemDetailRef"
    :alert-box-ref="alertBox"
    @edit-problem="handleEditFromDetail"
  />
  ...
  <transition name="dropdown">
    <div v-if="showAdvancedSearch" class="advanced-search">
      <div class="advanced-search-header">
        ...
        <el-button type="text" class="reset-btn" @click="resetAdvancedSearch">
          <el-icon>
            <Refresh />
          </el-icon>
          重置筛选
  ...
  <div v-if="selectedProblems.length > 0" class="batch-operations">
          ...
          <div class="pass-rate">
            ...
            <div class="progress-bar">
              <div
                class="progress"
                :style="{
                  width: scope.row.passRate + '%',
                  backgroundColor: getPassRateColor(scope.row.passRate),
                }"
              ></div>
            </div>
          </div>
  ...
  <div class="pagination-container">
    <div class="pagination">
      ...
      <div class="page-numbers">
        ...
        <!-- 显示当前页附近的页码 -->
        <button
          v-for="num in displayedMiddlePages"
          :key="num"
          class="page-number"
...
const problemCreateRef = ref<any>(null);
const problemEditRef = ref<any>(null);
const problemDetailRef = ref<any>(null);
...
const fetchData = async (): Promise<void> => {
  ...
  try {
    ...
    const formattedProblems = apiData.questions.map((item: any) => ({
      ...
      passRate: item.submit_num > 0 ? Math.floor((item.solve_num / item.submit_num) * 100) : 0,
    ...
    // totalPages 是由 totalProblems/pageSize 派生的 computed，无需赋值
  } catch (error) {
...
const getTagType = (tag: string) => {
  switch (tag) {
    case '入门':
      return 'success'; // 对应 :deep(.el-tag--success)
    case '普及':
      return 'warning'; // 对应 :deep(.el-tag--warning)
    case '提高':
      return 'info'; // 对应 :deep(.el-tag--info)
    case '省选':
      return 'primary'; // 对应 :deep(.el-tag--primary)
    case 'NOI':
      return 'danger'; // 对应 :deep(.el-tag--danger)
    case 'CTSC':
      return 'ctsc'; // 对应 :deep(.el-tag--ctsc)
    default:
      return 'info';
  }
};
...
const deleteProblem = (id: string) => {
  ElMessageBox.confirm('确定要删除这个题目吗？此操作不可逆。', '删除确认', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
  })
...
const displayedMiddlePages = computed(() => {
  ...
  if (total <= 7) {
    ...
    return Array.from({ length: total - 2 }, (_, i) => i + 2).filter(
...
:deep(.el-date-table td.start-date span, .el-date-table td.end-date span) {
  background-color: #18a058 !important;
}

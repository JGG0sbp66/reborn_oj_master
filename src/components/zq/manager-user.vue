<template>
  <div class="page-header">
    ...
    <div class="header-right">
      ...
      <race-create ref="raceCreateRef" :alert-box-ref="alertBox" @refresh-data="fetchData" />
      <race-edit ref="raceEditRef" :alert-box-ref="alertBox" @refresh-data="fetchData" />
      <race-detail
        ref="raceDetailRef"
        :alert-box-ref="alertBox"
        @refresh-data="fetchData"
        @edit-race="handleEditFromDetail"
      />
    </div>
  </div>
  ...
  <div v-if="selectedCompetitions.length > 0" class="batch-operations">
      ...
      <el-table-column label="操作" width="180" fixed="right" align="center">
        <template #default="scope">
          <div class="action-buttons">
            ...
            <el-button type="primary" size="small" text @click="manageUser(scope.row)">
              管理
            </el-button>
          </div>
        </template>
  ...
  >
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" :loading="banSubmitting" @click="submitBanUser"
          >确认封禁</el-button
        >
      </div>
    </template>
...
// Element Plus MessageBox 不支持直接绑定 Vue 事件，
// 用户详情弹窗的分页按钮通过 window 上的临时全局函数回调，这里做类型声明
declare global {
  interface Window {
    __qPrevPage?: () => void;
    __qNextPage?: () => void;
...
const batchChangeStatus = () => {
  ElMessageBox.prompt('请选择新的用户状态', '批量修改状态', {
    ...
    .then(({ value }) => {
      const ids = selectedCompetitions.value
        .map((competition: FormattedCompetition) => competition.id)
        .join(', ');
      if (alertBox.value) {
...
const filteredCompetitions = computed(() => {
  ...
  return competitions.value.filter((user: any) => {
    ...
    const matchesSearch = searchQuery.value
      ...
      : true;
    // 状态过滤
    const matchesStatus = statusFilter.value ? user.rating === statusFilter.value : true;
    // 其他过滤条件可根据需要添加
...
const manageUser = (user: any) => {
  ...
  const roleText =
    userRole === 'admin' ? '管理员' : userRole === 'superAdmin' ? '超级管理员' : '普通用户';
  const roleClass =
    userRole === 'admin'
      ? 'admin-role'
      : userRole === 'superAdmin'
  ...
  ElMessageBox.alert(`<div class="user-detail-content">加载中...</div>`, '用户信息', {
